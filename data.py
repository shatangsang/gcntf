...
class Dataloader(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for i in range(neighbor.shape[1]):
            neighbor_traj = neighbor[0, i].cpu().numpy()
            neighbor_traj = neighbor_traj[~np.all(neighbor_traj == 1e9, axis=1)]
            lcs_score = self.lcs(B,neighbor_traj)
            lcs_scores.append(lcs_score/8)
        return x, y, neighbor
        ...
        while tid0 < e-horizon:
            ...
            if len(idx):
                ...
                for i in range(len(idx)):
                    ...
                    neighbor = agents[:valid_horizon, [d for d in range(agents.shape[1]) if d != i]]
