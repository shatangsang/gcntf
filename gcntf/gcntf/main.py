...
if __name__ == "__main__":
    ...
    if settings.test:
        ...
        def test(model, fpc=1):
            ...
            with torch.no_grad():
                for x, y, neighbor, *_ in test_data:
                    batch += x.size(1)
    ...
    model = gcntf(horizon=config.PRED_HORIZON, ob_radius=config.OB_RADIUS, hidden_dim=config.RNN_HIDDEN_DIM)
